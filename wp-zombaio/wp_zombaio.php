...
define('WP_ZOMBAIO_VERSION', '1.0.7');
    ...
    protected function setup() {
        ...
        */
        ...
        $this->chargeback_liability_code = array(
            '',
            __('Merchant is liable for the chargeback', 'wp-zombaio'),
...
';
    ...
    */
    ...
    public function admin_enqueue_scripts() {
        ...
        if (isset($_GET['page']) && substr($_GET['page'], 0, 10) == 'wp_zombaio') {
            ...
            wp_enqueue_style('jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.10.1/themes/base/jquery-ui.css');
    ...
    */
    public function admin_page() {
        ...
        if ($do == 'wizard') {
            include(__DIR__ . '/wp_zombaio_wizard.php');
        }
        ...
        if (!$do) {
            ...
            echo '<p>' . __('For Reference, your Zombaio Postback URL (ZScript) should be set to', 'wp-zombaio') . ' ' . $this->_postbackurl() . '</p>';
            echo '<table>';
            ...
            */
            ...
            echo '<tr><th valign="top">' . __('Bypass IPN Verfication', 'wp-zombaio') . '</th><td valign="top">';
                ...
                <div class="error"><h4>' . __('We Validate the Zombaio IP against a Known list, You can bypass this if needed', 'wp-zombaio') . '</h4></div>
                ';
...
';
    ...
    private function _postbackurl() {
        return '<input type="text" name="postbackurl" value="' . site_url() . '" onclick="jQuery(this).select();" style="width: 250px;" readonly="readonly" />';
    }
...
';
    ...
    */
    ...
    /**
    Detect
    Looks for IP loading
    And for Processor In the Holing
    */
    public function detect() {
        ...
        if ($gw_pass) {
            require_once(plugin_dir_path(__FILE__) . 'wp_zombaio_processor.php');
            $wp_zombaio = new wp_zombaio_processor();
            $wp_zombaio->process();
        }
    }
