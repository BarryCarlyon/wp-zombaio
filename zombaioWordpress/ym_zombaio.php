...
/*
* $Id: ym_zombaio.php 2280 2012-08-08 08:51:56Z bcarlyon $
* $Revision: 2280 $
* $Date: 2012-08-08 09:51:56 +0100 (Wed, 08 Aug 2012) $
*/
...
//			echo 'Error in IPN. Missing ZombaioGWPass';
		...
		if ($gw_pass != $this->gw_pass) {
			header('HTTP/1.0 401 Unauthorized');
//			echo 'Error in IPN. Invalid ZombaioGWPass';
			echo '<h1>Zombaio Gateway 1.1</h1><h3>Authentication failed.</h3>';
			exit;
		}
		...
		// verify site ID
		$site_id = ym_request('SITE_ID');
		if ($site_id && $site_id != $this->site_id) {
			header('HTTP/1.0 401 Unauthorized');
//			echo 'Error in IPN. Invalid Site ID';
			echo '<h1>Zombaio Gateway 1.1</h1><h3>Authentication failed.</h3>';
			exit;
		}
